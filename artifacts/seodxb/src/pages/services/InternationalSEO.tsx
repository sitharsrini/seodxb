...
const intlServices = [
  { icon: <Globe size={22} />, title: "Hreflang Implementation", desc: "Correct hreflang tags signal to Google which version of your content targets which country and language — preventing duplicate content penalties across regions." },
...
const markets = [
  ...
  { region: "Asia Pacific", countries: "Australia, Singapore, India, Japan, UAE" },
      ...
      <div className="pt-24">
        ...
        <section className="bg-slate-950 text-white py-24">
          <div className="container mx-auto px-4 max-w-4xl text-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <span className="inline-block bg-primary/20 text-primary text-xs font-bold tracking-widest uppercase px-4 py-2 rounded-full mb-6">International SEO</span>
              <h1 className="text-5xl md:text-6xl font-heading font-black mb-6 leading-tight">
                Rank in Every Market<br />
                <span className="text-[#C8FF00]">You Serve. Anywhere.</span>
              </h1>
        ...
        <section className="py-16 bg-slate-900 text-white">
          <div className="container mx-auto px-4 max-w-5xl">
            <p className="text-center text-white/50 text-xs uppercase tracking-widest mb-8">Markets We Serve</p>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {markets.map((m, i) => (
                <div key={i} className="bg-slate-800/50 rounded-xl p-5 border border-slate-700">
                  ...
                  <p className="text-xs text-white/50 leading-relaxed">{m.countries}</p>
                </div>
              ))}
            </div>
          </div>
