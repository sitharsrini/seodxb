...
class PageErrorBoundary extends Component<{ children: ReactNode }, { err: string | null }> {
  ...
  static getDerivedStateFromError(e: Error) {
    return { err: e.message || "Page failed to render" };
  }
  render() {
    if (this.state.err) {
      return (
        <div className="container mx-auto px-4 py-24 text-center">
          ...
          <p className="text-gray-500 mb-6">Refresh once. If it happens again, open the page in a new tab.</p>
          <a href="/" className="text-primary font-semibold underline">Go to home</a>
        </div>
      );
    }
...
function RedirectPricing() {
  if (typeof window !== "undefined") window.location.replace("/pricing");
...
function currentPath(loc: string) {
  const raw = typeof window !== "undefined" ? window.location.pathname : loc;
  return raw.replace(/\/+$/, "") || "/";
}

function Router() {
  const [loc] = useLocation();
  const [, setPop] = React.useState(0);
  React.useEffect(() => {
    const onPop = () => setPop((n) => n + 1);
    window.addEventListener("popstate", onPop);
    return () => window.removeEventListener("popstate", onPop);
  }, []);
  const path = currentPath(loc);
  const blogSlug = path.startsWith("/blog/") ? path.slice("/blog/".length) : "";
  return (
    <PageErrorBoundary key={path}>
    <Suspense fallback={<div className="min-h-[50vh] flex items-center justify-center text-gray-400 text-sm">Loading…</div>}>
    {path === "/blog" ? (
      <Blog />
    ) : blogSlug && !blogSlug.includes("/") ? (
      <BlogPost params={{ slug: blogSlug }} />
    ) : (
    <Switch>
      ...
      <Route path="/seo-packages" component={RedirectPricing} />
...
function Shell() {
  ...
  const isHome = path === "/";
  return (
    <div className={isHome ? "" : "min-h-screen flex flex-col font-sans selection:bg-primary selection:text-white"}>
      <RouteSeo />
      {!isHome && <Navbar />}
