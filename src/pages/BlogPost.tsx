...
import { imageForPage, absoluteImage } from "@/lib/pageImages";
...
declare global {
  interface Window {
    __BLOG_DATA__?: InjectedBlog;
  }
}

export function BlogPost({ params: routeParams }: { params?: { slug?: string } }) {
  const ctxParams = useParams<{ slug: string }>();
  const pathSlug = typeof window !== "undefined"
    ? window.location.pathname.replace(/\/+$/, "").match(/^\/blog\/([^/]+)$/)?.[1]
    : undefined;
  const slug = pathSlug || routeParams?.slug || ctxParams.slug;
  const post = slug ? posts[slug] : null;
  const injected = typeof window !== "undefined" && window.__BLOG_DATA__?.slug === slug
    ...
    return (
      <>
        <Helmet prioritizeSeoTags>
          <title>{clampTitle(`${injected.title} | SEODXB Blog`)}</title>
          <meta name="description" content={injected.excerpt} />
          <link rel="canonical" href={`https://seodxb.com/blog/${slug}`} />
        </Helmet>
        <div className="pt-28 pb-24 min-h-screen bg-white">
          <div className="container mx-auto px-4 max-w-3xl">
            <a href="/blog" className="inline-flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-primary transition-colors mb-10 group">
            ...
            <h1 className="text-4xl md:text-5xl font-black tracking-[-0.03em] leading-[1.05] mt-4 mb-6">{injected.title}</h1>
            <div className="flex items-center gap-5 text-sm text-gray-400 mb-8 flex-wrap">
              <span className="flex items-center gap-1.5"><Calendar size={14} /> {injected.date}</span>
              <span className="flex items-center gap-1.5"><Clock size={14} /> {injected.time}</span>
            </div>
            <div className="mb-10 rounded-3xl overflow-hidden aspect-[16/7] bg-gray-100">
              <img
                src={injected.imageUrl || imageForPage({ slug: slug || "", category: injected.category, title: injected.title }).src}
                alt={injected.title}
            ...
            <div
              className="prose prose-lg max-w-none text-gray-700 leading-relaxed
                [&_h1]:hidden
                [&_h2]:text-2xl [&_h2]:font-black [&_h2]:text-black [&_h2]:mt-10 [&_h2]:mb-4
            ...
            <div className="mt-10 text-center">
              <Button asChild variant="outline" className="rounded-full px-6">
                <a href="/blog"><ArrowLeft size={14} className="mr-2" /> All Articles</a>
              </Button>
            </div>
          </div>
        </div>
  ...
  return (
    <>
      <Helmet prioritizeSeoTags>
        ...
        <meta property="og:image" content={absoluteImage(post.imageUrl || imageForPage({ slug: slug || "", category: post.category, title: post.title }).src)} />
        ...
        <meta name="twitter:image" content={absoluteImage(post.imageUrl || imageForPage({ slug: slug || "", category: post.category, title: post.title }).src)} />
    ...
    <div className="pt-28 pb-24 min-h-screen bg-white">
      <div className="container mx-auto px-4 max-w-3xl">
        ...
        >
          ...
          <div className="mb-10 rounded-3xl overflow-hidden aspect-[16/7] bg-gray-100">
            <img
              src={post.imageUrl || imageForPage({ slug: slug || "", category: post.category, title: post.title }).src}
              alt={post.title}
              className="w-full h-full object-cover"
              loading="eager"
              fetchPriority="high"
