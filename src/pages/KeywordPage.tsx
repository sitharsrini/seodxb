...
import { resolvePageCopy } from "@/lib/keywordLongform";
    ...
    <section className="py-20 bg-slate-50 border-t border-gray-100">
      <div className="container mx-auto px-4 max-w-5xl">
        ...
        <div className="text-center mt-8 flex flex-wrap justify-center gap-x-6 gap-y-2 text-sm font-semibold text-primary">
          <a href="/seo-services-dubai" className="hover:underline">All SEO services</a>
          <a href="/seo-agency-uae" className="hover:underline">SEO across the UAE</a>
          <a href="/blog" className="hover:underline">SEO insights &amp; guides</a>
        </div>
      </div>
    </section>
  );
...
export function KeywordPage() {
  ...
  if (config === undefined) {
    return (
      <div className="min-h-[50vh] flex items-center justify-center text-gray-400 text-sm" aria-busy="true">
        Loading…
      </div>
    );
  }
  ...
  const copy = resolvePageCopy(config);
  ...
  return (
    <>
      <Helmet prioritizeSeoTags>
        ...
        <meta property="og:image" content={absoluteImage(config.imageUrl || imageForPage({ slug, keyword: config.keyword, title: config.h1 }).src)} />
        ...
        <meta name="twitter:image" content={absoluteImage(config.imageUrl || imageForPage({ slug, keyword: config.keyword, title: config.h1 }).src)} />
      ...
      <div className="pt-24">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <Button size="lg" className="rounded-full bg-primary text-white hover:bg-primary/90 font-bold px-8" asChild>
                    <a href="/contact">{config.ctaButton ?? "Get Free Consultation"} <ArrowRight size={16} className="ml-2" /></a>
                  </Button>
                  <Button size="lg" variant="outline" className="rounded-full border-gray-300 text-gray-700 hover:bg-gray-50 px-8" asChild>
                    <a href="/seo-audit">Free SEO Audit</a>
                  </Button>
                </div>
              </motion.div>
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
                <img
                  src={config.imageUrl || imageForPage({ slug, keyword: config.keyword, title: config.h1 }).src}
                  alt={config.h1}
        ...
        {/* Page-specific long-form copy: unique place + industry + service, 1200+ words */}
                ...
                <div key={block.heading} className={i > 0 ? "mt-12" : ""}>
                  ...
                  {block.h3s?.map((s) => (
                    <div key={s.title} className="mt-6">
                      <h3 className="text-xl font-heading font-bold mb-2 text-gray-900">{s.title}</h3>
                      <p className="text-gray-600 leading-relaxed mb-4">{s.text}</p>
                    </div>
                  ))}
                  {block.h4s?.map((s) => (
                    <div key={s.title} className="mt-4 ml-0">
                      <h4 className="text-lg font-heading font-semibold mb-1 text-gray-800">{s.title}</h4>
        ...
        {/* Why SEODXB — copy is unique to this place and keyword */}
        <section className="py-20 bg-slate-50">
          <div className="container mx-auto px-4 max-w-4xl">
            <h2 className="text-3xl font-heading font-bold mb-10 text-center">
              Why {copy.facts.place.name} businesses hire SEODXB
            </h2>
