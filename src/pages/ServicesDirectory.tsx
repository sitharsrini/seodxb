  ...
  return (
    <section className="mb-10">
      ...
      <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-2">
        {pages.map((p) => (
          <li key={p.slug}>
            <a href={`/${p.slug}`} className="text-primary hover:underline text-sm leading-snug block">
              {p.keyword}
            </a>
          </li>
        ))}
  ...
  return (
    <>
      <Helmet prioritizeSeoTags>
        <title>{clampTitle("SEO Services Directory - All Locations and Industries | SEODXB")}</title>
        <meta
          name="description"
