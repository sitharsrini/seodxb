...
import { longformHtml } from "../src/lib/keywordLongform";
...
import { longFormMeta, longFormContent, longFormSchemaData, longFormArticles } from "../src/data/longform";
import { imageForPage, absoluteImage } from "../src/lib/pageImages";
...
function buildHead(page: KeywordPageConfig, shell: string): string {
  ...
  return (
    shell
      .replace(
        /<title[^>]*>[^<]*<\/title>/,
        `<title data-rh="true">${escape(clampTitle(page.title))}</title>`,
      )
      .replace(
        /<meta[^>]*name="description"[^>]*\/>/,
        `<meta data-rh="true" name="description" content="${escape(page.metaDesc)}" />`,
      )
      .replace(
        /<link[^>]*rel="canonical"[^>]*\/>/,
      ...
      .replace(
        ...
        `<meta data-rh="true" property="og:title" content="${escape(clampTitle(page.title))}" />`,
      )
      .replace(
        /<meta[^>]*property="og:description"[^>]*\/>/,
        `<meta data-rh="true" property="og:description" content="${escape(page.metaDesc)}" />`,
      )
      .replace(
      ...
      .replace(
        ...
        `<meta data-rh="true" name="twitter:title" content="${escape(clampTitle(page.title))}" />`,
      )
      .replace(
        /<meta[^>]*name="twitter:description"[^>]*\/>/,
        `<meta data-rh="true" name="twitter:description" content="${escape(page.metaDesc)}" />`,
      )
      .replace(
        /<meta[^>]*property="og:image"[^>]*\/>/,
        `<meta data-rh="true" property="og:image" content="${escape(absoluteImage(page.imageUrl || imageForPage({ slug: page.slug, keyword: page.keyword, title: page.h1 }).src))}" />`,
      )
  );
}
  ...
  const relatedLinks = `
    <nav aria-label="Related SEO services">
      ...
      <ul>
        ...
        <li><a href="${SITE}/on-page-seo">On-Page SEO</a></li>
        <li><a href="${SITE}/technical-seo">Technical SEO</a></li>
        <li><a href="${SITE}/aeo">AEO</a></li>
        <li><a href="${SITE}/geo">GEO</a></li>
        <li><a href="${SITE}/blog">SEO insights and guides</a></li>
        <li><a href="${SITE}/contact">Contact SEODXB</a></li>
      </ul>
    </nav>`;
  const article = longformHtml(page, escape);
  const hero = imageForPage({ slug: page.slug, keyword: page.keyword, title: page.h1 });
  const heroImg = `<img src="${escape(hero.src)}" alt="${escape(hero.alt)}" width="1200" height="675" />`;
  const staticBlock = `<div id="static-content" aria-hidden="true" style="position:absolute;left:-9999px;top:0;width:1px;height:1px;overflow:hidden;">${heroImg}${article}${relatedLinks}</div>`;
  const noscript = `<noscript>${article}${relatedLinks}</noscript>`;
  return html.replace('<div id="root"></div>', `${staticBlock}\n<div id="root"></div>${noscript}`);
}
...
const STATIC_META: Record<string, { title: string; desc: string }> = {
  ...
  "privacy": { title: "Privacy Policy | SEODXB", desc: "How SEODXB collects, uses and stores personal information from seodxb.com and contact forms." },
  "terms": { title: "Terms of Service | SEODXB", desc: "Terms for using seodxb.com and for SEO, AEO and GEO work with SEODXB in Dubai." },
};
...
function buildStaticBody(slug: string): string {
  ...
  if (slug === "" || slug === "index") {
    inner = `
<h1>SEODXB Dubai: SEO, SME ERP websites, ecommerce and social media design</h1>
<p>SEODXB is a Dubai studio for SEO, AEO and GEO, SME ERP websites, ecommerce storefronts and social media design. We help businesses rank on Google and get cited by ChatGPT, Perplexity and Google AI Overviews.</p>
<p>Work includes SME ERP websites, ecommerce sites, search visibility, social media design and live storefronts.</p>
<nav aria-label="SEODXB work">
<ul>
<li><a href="${SITE}/on-page-seo">On-page SEO</a></li>
<li><a href="${SITE}/technical-seo">Technical SEO</a></li>
<li><a href="${SITE}/aeo">AEO</a></li>
<li><a href="${SITE}/geo">GEO</a></li>
<li><a href="${SITE}/ecommerce-seo">Ecommerce SEO</a></li>
<li><a href="${SITE}/b2b-seo">B2B SEO</a></li>
<li><a href="${SITE}/seo-dubai">SEO Dubai</a></li>
<li><a href="${SITE}/website-20-aed">Live storefronts</a></li>
<li><a href="${SITE}/contact">Contact</a></li>
<li><a href="${SITE}/blog">Journal</a></li>
</ul>
</nav>
<section>
<h2>Questions</h2>
<h3>What can you design?</h3><p>SME ERP websites, ecommerce storefronts, search systems, and social media design. One studio, one visual language.</p>
<h3>Do you ship live websites?</h3><p>Yes. Design and build land together so the site you approve is the site that goes live.</p>
<h3>How fast can we start?</h3><p>Most briefs start within a week. A clear homepage and catalog can ship in 2 to 4 weeks.</p>
</section>
`;
  } else if (slug === "about") {
    inner = `
...
<p>SEODXB is a Dubai-based SEO agency. We collect name, email, phone, company URL and message when you submit a form, so we can reply. We do not sell personal data. Enquiries: hi@Listi.ae.</p>
`;
  } else if (slug === "terms") {
    inner = `
<h1>Terms of Service</h1>
<p>Using seodxb.com or buying SEO, AEO or GEO work from SEODXB is month-to-month unless a proposal says otherwise. Rankings are objectives, not guarantees. UAE law applies. Contact hi@Listi.ae.</p>
`;
...
function buildStaticHead(shell: string, url: string, title: string, descRaw: string, ogType = "website"): string {
  ...
  return shell
    .replace(/<title[^>]*>[^<]*<\/title>/, `<title data-rh="true">${escape(clampTitle(title))}</title>`)
    .replace(/<meta[^>]*name="description"[^>]*\/>/, `<meta data-rh="true" name="description" content="${escape(desc)}" />`)
    ...
    .replace(/<meta[^>]*property="og:title"[^>]*\/>/, `<meta data-rh="true" property="og:title" content="${escape(clampTitle(title))}" />`)
    .replace(/<meta[^>]*property="og:description"[^>]*\/>/, `<meta data-rh="true" property="og:description" content="${escape(desc)}" />`)
    .replace(/<meta[^>]*property="og:url"[^>]*\/>/, `<meta data-rh="true" property="og:url" content="${url}" />`)
    .replace(/<meta[^>]*name="twitter:title"[^>]*\/>/, `<meta data-rh="true" name="twitter:title" content="${escape(clampTitle(title))}" />`)
    .replace(/<meta[^>]*name="twitter:description"[^>]*\/>/, `<meta data-rh="true" name="twitter:description" content="${escape(desc)}" />`);
}
...
function blogInnerHtml(slug: string, title?: string): string {
  const c = ALL_BLOG_CONTENT[slug];
  if (!c) return "";
  const esc = (s: string) => s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  const parts: string[] = [];
  ...
  for (const sec of c.sections) {
    parts.push(`<h2>${esc(sec.h)}</h2>`);
    for (const para of sec.p) parts.push(`<p>${esc(para)}</p>`);
    if (sec.list) parts.push(`<ul>${sec.list.map((li) => `<li>${esc(li)}</li>`).join("")}</ul>`);
  }
  parts.push(`<p>${esc(c.takeaway)}</p>`);
  if (c.faqs && c.faqs.length > 0) {
    parts.push(`<h2>Frequently asked questions</h2>`);
    for (const f of c.faqs) parts.push(`<h3>${esc(f.q)}</h3><p>${esc(f.a)}</p>`);
  }
  return parts.join("\n");
}

const BLOG_PAGE_EXTRA: Record<string, { category: string; date: string; iso: string; updated: string; time: string; excerpt: string; imageUrl?: string }> = {
  ...Object.fromEntries(
    [...generatedBlogPosts, ...generatedBlogPosts2].map((p) => [p.slug, {
      category: p.category, date: p.date, iso: p.iso, updated: p.updated, time: p.time, excerpt: p.excerpt, imageUrl: p.imageUrl,
    }]),
  ),
  ...Object.fromEntries(
    longFormArticles.map((a) => [a.slug, {
      category: a.category, date: a.date, iso: a.iso, updated: a.updated, time: a.time, excerpt: a.excerpt, imageUrl: a.heroImage,
    }]),
  ),
};
...
function prerender() {
  ...
  for (const page of pages) {
    ...
    count++;
    if (count % 100 === 0) console.log(`  … ${count}/${pages.length}`);
  }
  console.log(`  ✓ ${count} keyword pages`);
  ...
  const REDIRECTED_STATIC = new Set(["seo-packages"]);
  ...
  for (const [route, meta] of Object.entries(STATIC_META)) {
    ...
    if (!existsSync(file)) {
      ...
      if (route === "seo-services") {
        ...
        const allPosts = Object.entries(BLOG_META) as [string, { title: string; desc: string }][];
        html = html.replace('<div id="root"></div>', `${buildBlogIndexBody(allPosts)}\n<div id="root"></div>`);
      } else {
  ...
  const listingFile = join(DIST, "blog.html");
  if (existsSync(listingFile)) {
    copyFileSync(listingFile, join(DIST, "blog", "index.html"));
  }
  let blogCount = 0;
  for (const [slug, meta] of Object.entries(BLOG_META)) {
    ...
    const extra = BLOG_PAGE_EXTRA[slug];
    const inner = blogInnerHtml(slug, h1Title);
    const hero = imageForPage({ slug, category: extra?.category, title: h1Title });
    const imageUrl = extra?.imageUrl || hero.src;
    if (imageUrl) {
      const imgTag = `<img src="${escape(imageUrl)}" alt="${escape(h1Title)}" width="1200" height="675" />`;
      bodyBlock = bodyBlock.replace("<article>", `<article>${imgTag}`);
    }
    if (extra && inner) {
      ...
      blogHtml = blogHtml.replace("</head>", `  <script>window.__BLOG_DATA__=${payload}</script>\n  </head>`);
    }
    blogHtml = blogHtml.replace('<div id="root"></div>', `${bodyBlock}\n<div id="root"></div>`);
    writeFileSync(join(DIST, "blog", `${slug}.html`), blogHtml, "utf-8");
    blogCount++;
    if (blogCount % 100 === 0) console.log(`  … ${blogCount} blog pages`);
  }
  console.log(`  ✓ ${Object.keys(STATIC_META).length} static + ${blogCount} blog articles`);
  ...
  const sitemapBlog = [
    ...
    ...Object.keys(BLOG_META).map((slug) =>
      `<url>\n    <loc>${SITE}/blog/${slug}</loc>\n    <lastmod>${today}</lastmod>\n    <changefreq>monthly</changefreq>\n    <priority>${CORE_BLOGS.has(slug) ? "0.7" : "0.5"}</priority>\n  </url>`,
    ),
    "</urlset>",
  ].join("\n");
  ...
  const fullLines: string[] = [
    "# SEODXB - Content Index",
    "",
    "> Unique keepers on seodxb.com. Farm clones 301 to these URLs.",
    ...
    "## Articles",
    "",
    ...Object.keys(BLOG_META).map((slug) => `- ${SITE}/blog/${slug}`),
    "",
    "## Contact",
    "",
    "- Email: hi@Listi.ae",
  ...
  const notFound = shell
    .replace(/<title[^>]*>[^<]*<\/title>/, '<title data-rh="true">Page Not Found | SEODXB</title>')
    .replace(
